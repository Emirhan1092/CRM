					...
					<div class="row align-items-center">
						<div class="col-md-12 col-sm-12">
							<div class="section-profile-boxes-alpha-item-heading">
								@php $candidate_name = $candidate['first_name'].' '.$candidate['last_name']; @endphp
								<a href="{{route('front-candidate-detail', ($candidate['slug'] ? $candidate['slug'] : encode($candidate['candidate_id'])))}}" class="btn" title="{{$candidate_name}}">
									<h2>{{trimString($candidate_name, 25)}}</h2>
								</a>
							</div>
							...
							<div class="section-profile-boxes-alpha-item-skills">
								@if($candidate['skill_titles'] && is_array($candidate['skill_titles']))
									...
									<span title="{{$st}}">{{ trimString($st, 12) }}</span>
									@endforeach
