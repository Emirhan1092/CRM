...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">{{__('message.employers')}}</h1>
                </div>
                <!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">{{__('message.home')}}</a></li>
                        <li class="breadcrumb-item active">{{__('message.employers')}}</li>
                    </ol>
                </div>
                <!-- /.col -->
            </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{__('message.employers').' '.__('message.list')}}</h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            ...
                            <div class="row dt-controls-row">
                                <div class="col-md-12">
                                    <div class="datatable-top-controls datatable-top-controls-filter">
                                        @if(allowedTo('create_employer'))
                                        <button type="button" class="btn btn-info create-or-edit-employer">
                                        <i class="fa fa-plus"></i> {{ __('message.add_employer') }}
                                        </button>
                                        @endif
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                ...
                                                <a class="dropdown-item bulk-action" href="#" data-action="download-excel">{{ __('message.download_employers_excel') }}</a>
                                            </div>
                                        </div>                                        
                                    </div>
                                    <div class="datatable-top-controls datatable-top-controls-dd">
                                        <div class="input-group input-group-sm">
                                            <select class="form-control select2" id="role">
                                                ...
                                                <option value="{{ esc_output($value['role_id']) }}">{{ esc_output($value['title'], 'html') }}</option>
                                                @endforeach
                                            </select>
                            ...
                            <table class="table table-bordered table-striped" id="employers_datatable">
                                <thead>
                                    <tr>
...
<!-- Forms for actions -->
<form id="employers-form" method="POST" action="{{url(route('admin-employers-excel'))}}" target='_blank'></form>

@endsection
@section('page-scripts')
<script src="{{url('admins')}}/js/cf/employer.js"></script>
@endsection