...
class JobBoardController extends Controller
{
    ...
    public function candidateStatus(Request $request)
    {
        ...
        JobBoard::updateCandidateStatus($data);

        //Sending status change email to all candidates
        $ids = decodeArray($data['ids']);
        $job = objToArr(Job::getJob('jobs.job_id', $data['job']));
        foreach ($ids as $candidate_id) {
            $candidate = Candidate::getCandidate('candidates.candidate_id', $candidate_id);
            $tagsWithValues = array(
                '((site_link))' => url('/'.employerId('slug')),
                '((site_logo))' => settingEmp('site_logo'),
                '((site_name))' => settingEmp('site_name'),
            ...
            $messageEmail = replaceTagsInTemplate2(settingEmp('job_application_status_change'), $tagsWithValues);
            $subject = settingEmp('site_name').' : '.__('message.job_application_status_change');
            $this->sendEmail($messageEmail, $candidate['email'], $subject);
        }
