...
class MembershipsController extends Controller
{
    ...
    */
    ...
    /**
    * Function for flutterwave payment
    *
    * @param $selected string
    * @return void
    */
    public function flutterwavePayment(Request $request, $selected = NULL)
    {
        $selected = $this->selectedBreakup($selected);
        \Session::put(array('flutterwave_selected' => $selected));
        $email = employerSession('email');
        $name = employerSession('first_name').' '.employerSession('last_name');
        ...
        $request = array(
            ...
            'redirect_url' => route('employer-payment-flutterwave-process'),
            'customer' => array(
                'email' => $email,
                'name' => $name
            ),
        ...
        if ($response->status == 'success') {
            ...
            die('We can not process your payment');
        }
    }

    /**
    * Function for flutterwave payment process
    *
    * @return void
    */
    public function flutterwavePaymentProcess(Request $request)
    {
        if (isset($_GET['status'])) {
            if ($_GET['status'] == 'cancelled') {
                ...
                $response = curl_exec($curl);
                curl_close($curl);
                $response = objToArr(json_decode($response));
                $err = curl_error($curl);
                if ($err) {
                  die('Curl returned error: ' . $err);
                }
                if ($response['status'] == 'success') {
                    ...
                    $amount_to_pay = $response['data']['meta']['price'];
                    if ($amount_paid >= $amount_to_pay) {
                        $sel = \Session::get('flutterwave_selected');
                        $package = $sel['package'];
                        $response['balance_transaction'] = $response['data']['tx_ref'];
                        $response['currency'] = $sel['package']['currency_for_api'];
                        $response['status'] = 1;
                        $this->addMembershipToDb($response, $sel['package_title'], $sel['package_id'], $sel['type'], $sel['amount'], employerId(), 'flutterwave');
                        $message = __('message.payment_successfull')." (". $response["balance_transaction"].")";
                        echo json_encode(array('success' => 'true', 'messages' => $message,));
                        return redirect(route('employer-memberships'));
                    } else {
                        echo "Fraud Transation Detected";
                    }
                } else {
    ...
    /**
    * Function for mollie payment
    *
    * @param $selected string
    * @return void
    */
    public function molliePayment(Request $request, $selected = NULL)
    {
        //Getting ticket detail
        $selected = $this->selectedBreakup($selected);
        $key = setting('mollie_environment') == 'live' ? setting('mollie_live_api_key') : setting('mollie_test_api_key');
        ...
        $payment = $mollie->payments->create([
            "amount" => [
                "currency" => strtoupper($selected['package']['currency_for_api']),
                "value" => number_format((float)$selected['amount'], 2, '.', ''),
            ],
            "description" => $selected['package_title'],
            "redirectUrl" => route('employer-payment-mollie-success'),
            "metadata" => [
                "order_id" => time(),
            ],
        ]);

        \Session::put(array('mollie_payment_id' => $payment->id));
        \Session::put(array('mollie_selected' => $selected));
    ...
    */
    public function mollieSuccess(Request $request)
    {
        ...
        $mollie = new \Mollie\Api\MollieApiClient();
        $mollie->setApiKey($key);
        $response = $mollie->payments->get($paymentId);
        if ($response->isPaid()) {
            $sel = \Session::get('mollie_selected');
            $response = objToArr($response);
            $response['balance_transaction'] = $response['id'];
            $response['currency'] = $sel['package']['currency_for_api'];
            $response['status'] = 1;
            $this->addMembershipToDb($response, $sel['package_title'], $sel['package_id'], $sel['type'], $sel['amount'], employerId(), 'mollie');
            $message = __('message.payment_successfull')." (". $response["balance_transaction"].")";
            echo json_encode(array('success' => 'true', 'messages' => $message,));
            return redirect(route('employer-memberships'));
        } else {
            return redirect()->route('payment-mollie-cancel');            
        }
    }

    /**
    * Function for mollie cancel
    *
    * @return void
    */
    public function mollieCancel(Request $request)
    {
