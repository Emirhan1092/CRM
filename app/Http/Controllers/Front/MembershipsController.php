    ...
    */
    public function flutterwavePayment(Request $request, $selected = NULL)
    {
        ...
        $request = array(
            ...
            'redirect_url' => route('candidate-payment-flutterwave-process'),
            'customer' => array(
                'email' => $email,
                'name' => $name
            ),
            ...
            if ($_GET['status'] == 'cancelled') {
                ...
                if ($response['status'] == 'success') {
                    ...
                    if ($amount_paid >= $amount_to_pay) {
                        ...
                        $response['status'] = 1;
                        $this->addMembershipToDb($response, $sel['package_title'], $sel['candidate_package_id'], $sel['type'], $sel['amount'], candidateSession(), 'flutterwave');
                        $message = __('message.payment_successfull')." (". $response["balance_transaction"].")";
                        echo json_encode(array('success' => 'true', 'messages' => $message,));
                        return redirect(route('front-acc-memberships-view'));
                    } else {
    ...
    */
    public function molliePayment(Request $request, $selected = NULL)
    {
        ...
        $payment = $mollie->payments->create([
            ...
            "redirectUrl" => route('candidate-payment-mollie-success'),
            "metadata" => [
                "order_id" => time(),
            ],
        ]);
    ...
    */
    public function mollieSuccess(Request $request)
    {
        ...
        if ($response->isPaid()) {
            ...
            $response['currency'] = $sel['package']['currency_for_api'];
            $response['status'] = 1;
            $this->addMembershipToDb($response, $sel['package_title'], $sel['candidate_package_id'], $sel['type'], $sel['amount'], candidateSession(), 'mollie');
            $message = __('message.payment_successfull')." (". $response["balance_transaction"].")";
            echo json_encode(array('success' => 'true', 'messages' => $message,));
            return redirect(route('front-acc-memberships-view'));
        } else {
