...
class GeneralsController extends Controller
{
    ...
    public function pricing(Request $request)
    {
        ...
        $data['packages'] = Package::getAll();
        $data['faqs'] = Faq::getForPage('pricing');
        return view('front'.viewPrfx().'pricing', $data);
    }

    /**
     * View function to display faqs page
     *
     * @return html/string
     */
    public function faqs(Request $request)
    {
        ...
        //Checking if faq page is in menu
        if (!Menu::itemInMenu('faqs')) {
            die(__('message.not_allowed'));
        }

        $data['page_title'] = __('message.faqs');
        $data['page_link'] = 'faqs';
        $data['faqs'] = $this->sortFaqsCategoryWise(Faq::getForPage('faqs'));
        return view('front'.viewPrfx().'faqs', $data);
    }
    ...
    private function sortFaqsCategoryWise($faqs)
    {
        $return = array();
        foreach ($faqs as $faq) {
            $return[$faq['category']][] = $faq;
        }
        return $return;
    }
