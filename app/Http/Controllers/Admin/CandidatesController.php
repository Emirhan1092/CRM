...
class CandidatesController extends Controller
{
    ...
    public function message(Request $request)
    {
        ...
        $msg = sanitizeHtmlTemplates(templateInput('msg'));

        $rules['msg'] = ['required', new MinString(10), new MaxString(50000)];
        ...
        foreach ($candidates as $candidate_id) {
            $candidate = objToArr(Candidate::getCandidate('candidate_id', $candidate_id));
            $this->sendEmail(removeUselessLineBreaks($msg), $candidate['email'], $data['subject']);
        }
    ...
    public function loginAs($candidate_id, $user_id)
    {
        ...
        $sd = env('BFCRM_ROUTE_SLUG') == 'slug' ? '' : '-sd';
        $slugOrSubDomainSlug = env('BFCRM_ROUTE_SLUG') == 'slug' ? array('slug' => $slug) : array('subdomain_slug' => $slug);
