...
class CandidatesController extends Controller
{
    ...
    {
        //Checking google reCaptcha if enabled
        if (setting('recaptcha_enable_on_login') == 'yes') {
            $recaptcha = $request->input('g-recaptcha-response');
            $secret_key = setting('recaptcha_secret_key');
            $url = 'https://www.google.com/recaptcha/api/siteverify?secret='.$secret_key.'&response='.$recaptcha;
            $response = json_decode(file_get_contents($url));
            if (!$response->success) {
                return redirect(routeWithSlug('candidate-login-view'))->withErrors([__('message.recaptcha_error_msg')]);
            }
        }
    ...
    public function register(Request $request)
    {
        ...
        //Checking google reCaptcha if enabled
        if (setting('recaptcha_enable_on_register') == 'yes') {
            $recaptcha = $request->input('g-recaptcha-response');
            $secret_key = setting('recaptcha_secret_key');
            $url = 'https://www.google.com/recaptcha/api/siteverify?secret='.$secret_key.'&response='.$recaptcha;
            $response = json_decode(file_get_contents($url));
            if (!$response->success) {
                die(json_encode(array(
                    'success' => 'false',
                    'messages' => $this->ajaxErrorMessage(array('error' => __('message.recaptcha_error_msg')))
                )));
            }
        }
    ...
    public function updateProfile(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'city' => ['required', new MinString(2), new MaxString(100)],
            'country' => ['required', new MinString(2), new MaxString(100)],
            'dob' => 'required',
            'state' => ['required', new MinString(2), new MaxString(100)],
            'address' => ['required', new MinString(2), new MaxString(150)],
            'bio' => ['required', new MinString(2), new MaxString(10000)],
        ], [
            'first_name.required' => __('validation.required'),
            'first_name.min' => __('validation.min_string'),
            'first_name.max' => __('validation.max_string'),
