...
function membershipgReceiptThumb($image) {
    $data['error'] = url('/cdn-general').'/essentials/images/general-not-found.png';
...
function generalThumb($image) {
    $data['error'] = url('/cdn-general').'/essentials/images/avatar-not-found.png';
    $data['image'] = $image ? route('uploads-view', $image) : '';
    return $data;
}
}
...
function newsThumb($image) {
    $data['error'] = url('/cdn-general').'/essentials/images/news-not-found.png';
    $data['image'] = $image ? route('uploads-view', $image) : '';
    return $data;
}
}
...
function employerThumb($image, $forLogo = false) {
    ...
    $data['error'] = url('/cdn-general').'/essentials/images/'.$not_found_image;
    $data['image'] = $image ? route('uploads-view', $image) : '';
    return $data;
}
}
...
function departmentThumb($image) {
    $data['error'] = url('/cdn-general').'/essentials/images/department-not-found.png';
    $data['image'] = $image ? route('uploads-view', $image) : '';
    return $data;
}
}
...
function footerColumns() {
    ...
    if ($data['col_1'] == '' && $data['col_2'] == '' && $data['col_3'] == '' && $data['col_4'] == '') {
        $all_columns_empty = true;
    }
    $footer['columns'] = $data;
    $footer['column_count'] = $count;
    $footer['all_columns_empty'] = $all_columns_empty;
    return $footer;
}
}
...
function empUrl() {
    $type = env('BFCRM_ROUTE_SLUG', 'slug');
    $url = env('APP_URL');
...
if(!function_exists('frontLanguage')) {
    function frontLanguage()
    {
        ...
        // Eğer seçili flag varsa, onu döndür
        if ($selected_flag) {
            return array(
                'flag' => $selected_flag,
            ...
            // Eğer default flag varsa, onu döndür, yoksa varsayılan bir değer kullan
            return array(
                'flag' => $default['flag'] ?? 'default_flag', // 'default_flag' yerine istediğiniz varsayılan değeri koyabilirsiniz
                'title' => $default['title'] ?? 'Default Title',
                'slug' => $default['slug'] ?? 'default_slug',
...
function viewPrfx($slash = false) {
    $prefix = env('BFCRM_FRONT_PRFX') ? env('BFCRM_FRONT_PRFX') : 'beta';
    if ($slash === 'only') {
        return $prefix;
    }
...
if (!function_exists('translating')) {
    function translating($html, $locale = null)
    {
        return preg_replace_callback(
            '/{{::(.*?)::}}/',
            function ($matches) use ($locale) {
                $key = trim($matches[1]);
                // 'message.' prefix eklenerek __() ile çeviri yapılır
                return __('message.' . $key, [], $locale);
            },
            $html
        );
    }
