...
class FaqsCategory extends Model
{
    ...
    protected static $tbl = 'faqs_categories';
    ...
    {
        $query = Self::whereNotNull('faqs_categories.category_id');
        if ($active) {
    ...
    {
        $columns = array(
            'faqs_categories.title',
            'faqs_categories.created_at',
            'faqs_categories.status',
        );
        ...
        if (isset($request['status']) && $request['status'] != '') {
            $query->where('faqs_categories.status', $request['status']);
        }
        $query->groupBy('faqs_categories.category_id');
    ...
    public static function getTotal($srh = false, $request = '')
    {
        ...
        if ($srh) {
            $query->where(function($q) use ($srh) {
                $q->where('faqs_categories.title', 'like', '%'.$srh.'%');
            });
        }
        if (isset($request['status']) && $request['status'] != '') {
            $query->where('faqs_categories.status', $request['status']);
    ...
    private static function prepareDataForTable($categories)
    {
        ...
        foreach ($categories as $u) {
            ...
            if (allowedTo('edit_faqs_categories') && strtolower($u['title']) != 'pricing') { 
            $actions .= '
                <button type="button" class="btn btn-primary btn-xs create-or-edit-faqs-category" data-id="'.$id.'"><i class="far fa-edit"></i></button>
            ';
            }
            if (allowedTo('delete_faqs_categories') && strtolower($u['title']) != 'pricing') { 
            $actions .= '
                <button type="button" class="btn btn-danger btn-xs delete-faqs-category" data-id="'.$id.'"><i class="far fa-trash-alt"></i></button>
            ';
            }
            $sorted[] = array(
                esc_output($u['title']),
                date('d M, Y', strtotime($u['created_at'])),
                '<button type="button" title="'.$button_title.'" class="btn btn-'.$button_class.' btn-xs change-faqs-category-status" data-status="'.$u['status'].'" data-id="'.$id.'">'.$button_text.'</button>',
                $actions ? $actions : '---'
            );
        }
        return $sorted;
