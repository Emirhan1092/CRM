...
class Employer extends Model
{
    ...
    public static function getEmployer($column, $value)
    {
        ...
        $query->leftJoin('industries', 'industries.industry_id', '=', 'employers.industry_id');
        $query = $query->first();
    ...
    public static function getAll($active = true, $limit = '', $type = 'main', $sort = '', $search = '', $industry = '', $no_of_employees = '')
    {
        ...
        if ($industry) {
            $query->where(function($q) use ($industry) {
                $q->where('employers.industry_id', decode($industry));
            });
        }
        if ($no_of_employees) {
    ...
    public static function importEmployerSettings($employer_id)
    {
        ...
        $data = array(
            ...
            array('employer_id' => $eid, 'category' => 'Emails', 'key' => 'job_application_status_change', 'value' => getTextFromFile('job-application-status-change.txt')),
